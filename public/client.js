...
function renderRoomView(state) {
    if (!state || !roomView || !myUserId) {
        console.error("[DEBUG] RenderRoomView PREVENTED: Invalid state, view, or no myUserId.", {stateExists: !!state, roomViewExists: !!roomView, myUserIdExists: !!myUserId});
        if (!myUserId && currentView === 'roomView') { handleLogout(); alert("用户身份丢失，请重新登录。"); }
        return;
    }
    console.log(`[DEBUG] renderRoomView START for room ${state.roomId}. MyUser: ${myUserId}. State.status: ${state.status}`);
    const myPlayerForLog = state.players.find(p => p.userId === myUserId);
    console.log(`[DEBUG] My player data in current state for renderRoomView:`, myPlayerForLog ? {name: myPlayerForLog.username, handLength: myPlayerForLog.hand?.length, handCount: myPlayerForLog.handCount, finished: myPlayerForLog.finished} : "NOT FOUND");

    // **** FORCE CLEAR #myHand AT THE VERY BEGINNING of renderRoomView ****
    const myHandContainerPreClear = document.getElementById('myHand');
    if (myHandContainerPreClear) {
        console.log(`[DEBUG] renderRoomView: #myHand PRE-CLEAR. Children: ${myHandContainerPreClear.children.length}. HTML: ${myHandContainerPreClear.innerHTML.substring(0, 50)}`);
        myHandContainerPreClear.innerHTML = ''; // Force clear
        console.log(`[DEBUG] renderRoomView: #myHand POST-CLEAR. Children: ${myHandContainerPreClear.children.length}. HTML: ${myHandContainerPreClear.innerHTML}`);
    } else {
    ...
    Object.values(playerAreas).forEach(clearPlayerAreaDOM); // This will also attempt to set a default for #myHand

    const myPlayer = state.players.find(p => p.userId === myUserId);
    if (!myPlayer) {
        console.error("[DEBUG] My player data NOT FOUND in game state during renderRoomView after clear!", state.players.map(p=>p.userId));
        // handleGameLeave(); // Avoid recursive leave if this is the issue
        return;
    }
    ...
    state.players.forEach(player => {
        ...
        if (targetArea) {
            renderPlayerArea(targetArea, player, isMe, state, player.slot); // This will call renderPlayerCards
        } else {
            console.warn(`[DEBUG] No target area for relative slot ${relativeSlot} (Player slot ${player.slot}, My slot ${mySlot})`);
        }
    });
...
function clearPlayerAreaDOM(area) {
    ...
    const cardsEl = area.querySelector('.playerCards'); // For #playerAreaBottom, this is #myHand
    ...
    if (cardsEl) {
        console.log(`[DEBUG] clearPlayerAreaDOM: Clearing .playerCards in ${area.id}. Children before: ${cardsEl.children.length}`);
        cardsEl.innerHTML = '<span style="color:#888; font-style:italic;">- 等待 -</span>';
        console.log(`[DEBUG] clearPlayerAreaDOM: .playerCards in ${area.id} set to "- 等待 -". Children after: ${cardsEl.children.length}`);
    } else {
        console.warn(`[DEBUG] clearPlayerAreaDOM: .playerCards NOT FOUND in area ${area.id}`);
    }
...
function renderPlayerArea(container, playerData, isMe, state, absoluteSlot) {
    // ... (renderPlayerArea unchanged from your previous full version, it will call renderPlayerCards)
...
function renderPlayerCards(container, playerData, isMe, isMyTurnAndPlaying) {
    ...
    if (isMe) {
        ...
        targetContainer.innerHTML = ''; // Also clear opponent's container
    ...
    if (isMe) {
        ...
        if (playerData.finished) {
            ...
            // This case should ideally be hit if hand is genuinely empty after clearing
            targetContainer.innerHTML = '<span style="color:#555; font-style:italic;">- 无手牌 -</span>';
            console.log("[DEBUG] renderPlayerCards: SELF has no cards, set to '- 无手牌 -'");
        } else {
            ...
            sortedHand.forEach((cardData, index) => {
                ...
                // ... (rest of card setup: selected, hinted, onclick)
        ...
        // ... (opponent hand rendering unchanged from your previous full version)
        if (playerData.finished) {
            targetContainer.innerHTML = '<span style="color:#888; font-style:italic;">已出完</span>';
...
// ... (event handlers like handleReadyClick, handleSortHand etc. unchanged from your previous full version) ...
...
socket.on('playerReadyUpdate', ({ userId, isReady }) => { if (currentGameState && currentView === 'roomView') { const player = currentGameState.players.find(p => p.userId === userId); if (player) { player.isReady = isReady; if (userId === myUserId) isReadyForGame = isReady; renderRoomView(currentGameState); } } }); // Simplified, renderRoomView will handle updates
...
socket.on('gameStarted', (initialGameState) => {
    console.log(`[EVENT] gameStarted received for room ${initialGameState.roomId}. My room: ${currentRoomId}`);
    if (currentView !== 'roomView' || currentRoomId !== initialGameState.roomId) {
        console.warn("[DEBUG] gameStarted: Not in the correct view or room. Ignoring.");
        return;
    }
    console.log('[DEBUG] gameStarted: Processing event. Initial state:', JSON.parse(JSON.stringify(initialGameState)));

    previousGameState = currentGameState ? JSON.parse(JSON.stringify(currentGameState)) : null;
    currentGameState = initialGameState; // IMPORTANT: Update currentGameState with the new state from server
    ...
    const selfHandContainer = document.getElementById('myHand');

    if (selfHandContainer && myPlayer && Array.isArray(myPlayer.hand)) {
        console.log(`[DEBUG] gameStarted: My hand has ${myPlayer.hand.length} cards. Container:`, selfHandContainer);
        selfHandContainer.innerHTML = ''; // Crucial clear before direct render

        // **** SIMPLIFIED DIRECT RENDERING for #myHand ****
        myPlayer.hand.sort(compareSingleCardsClient).forEach(card => { // Sort for consistent simple display
            const cardTextElement = document.createElement('span');
            cardTextElement.textContent = ` ${card.rank}${getSuitSymbol(card.suit)} `;
            cardTextElement.style.border = "1px solid black";
            cardTextElement.style.margin = "2px";
            cardTextElement.style.padding = "2px";
            cardTextElement.style.display = "inline-block"; // Make them appear side-by-side
            selfHandContainer.appendChild(cardTextElement);
        });
        console.log(`[DEBUG] gameStarted: SIMPLIFIED #myHand rendering complete. Children: ${selfHandContainer.children.length}`);

        // **** PARTIAL UI UPDATE (since full renderRoomView is skipped for this test) ****
        updateGameInfoBarDOM(currentGameState);
        updateGameStatusDisplayDOM(currentGameState); // Update status like "轮到你出牌"
        // Render opponents (using the existing full renderPlayerCards, which should be fine for opponents)
        currentGameState.players.forEach(player => {
            if (player.userId !== myUserId) {
                const mySlot = myPlayer.slot;
                let relativeSlot = (player.slot - mySlot + currentGameState.players.length) % currentGameState.players.length;
                const targetArea = playerAreas[relativeSlot];
                if (targetArea) {
                    const cardsEl = targetArea.querySelector('.playerCards');
                    if (cardsEl) renderPlayerCards(cardsEl, player, false, false);
                }
            }
        });
        renderCenterPileDOM(currentGameState); // Render center pile
        updateRoomControls(currentGameState);   // Update action buttons

    } else {
        console.error('[DEBUG] gameStarted: #myHand container, myPlayer, or myPlayer.hand is invalid. Cannot do simplified render.', {selfHandContainer, myPlayerExists: !!myPlayer, handIsArray: Array.isArray(myPlayer?.hand)});
        // Fallback to full render if direct simplified render fails for some reason
        console.log('[DEBUG] gameStarted: FALLING BACK TO FULL renderRoomView.');
        renderRoomView(currentGameState);
    }

    // **** DO NOT CALL FULL renderRoomView(currentGameState) here for this specific test ****
    // The goal is to see if #myHand is clean ONLY with the direct simplified render.
    // If this test passes (no重影), then the issue is within the full renderRoomView path for the self hand.
});

socket.on('gameStateUpdate', (newState) => { /* ... (gameStateUpdate unchanged, should use full renderRoomView) ... */ if (currentView === 'roomView' && currentRoomId === newState.roomId) { let myCurrentHand = null; if (currentGameState && currentGameState.players) { const myPlayerInOldState = currentGameState.players.find(p => p.userId === myUserId); if (myPlayerInOldState && myPlayerInOldState.hand && Array.isArray(myPlayerInOldState.hand)) { myCurrentHand = JSON.parse(JSON.stringify(myPlayerInOldState.hand)); } } previousGameState = currentGameState ? JSON.parse(JSON.stringify(currentGameState)) : null; currentGameState = newState; if (myCurrentHand && myCurrentHand.length > 0) { const myPlayerInNewStateUpdated = currentGameState.players.find(p => p.userId === myUserId); if (myPlayerInNewStateUpdated && !myPlayerInNewStateUpdated.finished && myPlayerInNewStateUpdated.hand === undefined) { console.warn("CLIENT: Restoring my hand locally as broadcast didn't include it for gameStateUpdate."); myPlayerInNewStateUpdated.hand = myCurrentHand; } } if (previousGameState && ( (previousGameState.currentPlayerId === myUserId && currentGameState.currentPlayerId !== myUserId) || (!currentGameState.lastHandInfo && previousGameState.lastHandInfo && currentGameState.currentPlayerId === myUserId) ) ) { selectedCards = []; clearHintsAndSelection(true); } renderRoomView(currentGameState); updateGameStatusDisplayDOM(currentGameState); } else if (currentRoomId && currentRoomId !== newState.roomId) { console.warn("Received gameStateUpdate for a different room. Ignoring."); } });
