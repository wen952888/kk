...
function displayMessage(element, message, isError = false, isSuccess = false) {
    if (element) {
        ...
        element.classList.remove('error', 'success', 'message');
        if (isError) element.classList.add('error');
        else if (isSuccess) element.classList.add('success');
        else if (element.id !== 'gameStatusDisplay') element.classList.add('message');
    }
}
...
// **** IMPORTANT: Ensure updateRoomControls is defined BEFORE renderRoomView ****
function updateRoomControls(state) {
    if (!state || !myUserId) {
        // console.warn("[DEBUG] updateRoomControls: Invalid state or no myUserId.");
        return;
    }
    const myPlayerInState = state.players.find(p => p.userId === myUserId);
    if (!myPlayerInState) {
        // console.warn("[DEBUG] updateRoomControls: My player data not found in state.");
        return;
    }
    ...
    if (readyButtonInstance) {
        ...
        console.warn("[DEBUG] updateRoomControls: #readyButton not found.");
    }
    ...
    if (actionsContainers.length > 0) {
        if (state.status === 'playing' && state.currentPlayerId === myUserId && !myPlayerInState.finished) {
            ...
            if(passTurnButton) {
                ...
                if (state.isFirstTurn && !state.lastHandInfo) {
                     const iAmD4Holder = myPlayerInState.hand && Array.isArray(myPlayerInState.hand) && myPlayerInState.hand.some(c => c.rank === '4' && c.suit === 'D');
        ...
        // console.warn("[DEBUG] updateRoomControls: .my-actions-container not found.");
    }
}
...
function renderRoomView(state) {
    ...
    console.log(`[DEBUG] My player data in current state for renderRoomView:`, myPlayerForLog ? {name: myPlayerForLog.username, handLength: myPlayerForLog.hand?.length, handCount: myPlayerForLog.handCount, finished: myPlayerForLog.finished, isReady: myPlayerForLog.isReady} : "NOT FOUND");

    const myHandContainerPreClear = document.getElementById('myHand');
    if (myHandContainerPreClear) {
        console.log(`[DEBUG] renderRoomView: #myHand PRE-CLEAR. Children: ${myHandContainerPreClear.children.length}.`);
        myHandContainerPreClear.innerHTML = '';
        console.log(`[DEBUG] renderRoomView: #myHand POST-CLEAR. Children: ${myHandContainerPreClear.children.length}.`);
    } else {
    ...
    if (!myPlayer) {
        console.error("[DEBUG] My player data NOT FOUND in game state for renderRoomView after clear!", {statePlayers: state.players.map(p=>p.userId), myUserId});
        return;
    }
    ...
    updateRoomControls(state); // THIS IS THE LINE THAT WAS CAUSING THE ERROR in previous log
...
// ... (The rest of the functions: clearPlayerAreaDOM, renderPlayerArea, fanCards, getCardImageFilename, renderCard, renderPlayerCards, handleRegister, handleLogin, etc. remain the same as the previous "good" version you provided. Ensure renderPlayerCards uses innerHTML='' for clearing #myHand)
...
socket.on('gameStarted', (initialGameState) => {
    ...
    renderRoomView(currentGameState); // Use full render now
});
            ...
            if (response.success) {
                ...
                if (response.roomState) {
                    ...
                    if (currentGameState.status === 'finished') {
                        if (currentGameState.gameResult || currentGameState.finalScores) {
                           console.log("[INIT] Reconnected to a FINISHED game, showing game over.");
                           showView('roomView');
                           renderRoomView(currentGameState);
                           showGameOver(currentGameState.gameResult || currentGameState);
                        } else {
                           console.log("[INIT] Reconnected to a FINISHED game (no specific result). Returning to lobby.");
                           handleReturnToLobby();
                        }
                    } else {
